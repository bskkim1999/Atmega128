...
//caculate time(timer/counter3)  //1초 지날때마다 1씩 증가함.
ISR(TIMER3_OVF_vect){
...
//timer/counter3, caculate time funtion
void INIT_timer_counter3(void){
	
	
	TCCR3B |= (1 << CS32); //pre-scaler : 256
	ETIMSK |= (1 << TOIE3); //accpet overflow interrupt
...
int main(void){
	...
	while (1) {
		...
		flag_ccw = 0;
		time_ = 0;
		//don't caculate time(every one second)!!!!
		ETIMSK &= ~(1 << TOIE3); //accpet overflow interrupt
		...
		if(wheels >= 0){
			INIT_INT4(); //external interrupt
			InitializeTimer1();  //fast pwm
			...
			INIT_timer_counter3(); //caculate time function, start time!(every one second)
